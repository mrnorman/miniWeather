...
void semi_discrete_step( double *state_init , double *state_forcing , double *state_out , double dt , int dir , double *flux , double *tend ) {
  ...
  std::for_each_n(std::execution::par_unseq,std::views::iota(0).begin(),(NUM_VARS*nz*nx),[=,nx=nx,nz=nz,hy_dens_cell=hy_dens_cell,i_beg=i_beg,k_beg=k_beg](int idx){
...
void compute_tendencies_x( double *state , double *flux , double *tend , double dt) {
  ...
  std::for_each_n(std::execution::par_unseq,std::views::iota(0).begin(),(NUM_VARS*nz*nx),[=,nx=nx,nz=nz](int idx){
    auto [i,k,ll] = idx3d(idx,nx,nz);
    int indt  = ll* nz   * nx    + k* nx    + i  ;
...
void compute_tendencies_z( double *state , double *flux , double *tend , double dt) {
  ...
  /////////////////////////////////////////////////
  //Compute fluxes in the x-direction for each cell
  std::for_each_n(std::execution::par_unseq,std::views::iota(0).begin(),((nz+1)*nx),[=,nx=nx,nz=nz,hy_dens_int=hy_dens_int,hy_dens_theta_int=hy_dens_theta_int,hy_pressure_int=hy_pressure_int](int idx){
...
void set_halo_values_x( double *state ) {
  ...
  if (nranks == 1) {
    ...
    std::for_each_n(std::execution::par_unseq,std::views::iota(0).begin(),(NUM_VARS*nz),[=,nx=nx,nz=nz](int idx){
    ...
    std::for_each_n(std::execution::par_unseq,std::views::iota(0).begin(),(NUM_VARS*nz*hs),[=,nx=nx,nz=nz,sendbuf_l=sendbuf_l,sendbuf_r=sendbuf_r](int idx){
    ...
    std::for_each_n(std::execution::par_unseq,std::views::iota(0).begin(),(NUM_VARS*nz*hs),[=,nx=nx,nz=nz,recvbuf_l=recvbuf_l,recvbuf_r=recvbuf_r](int idx){
      auto [s,k,ll] = idx3d(idx,hs,nz);
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      std::for_each_n(std::execution::par_unseq,std::views::iota(0).begin(),(nz*hs),[=,nx=nx,nz=nz,k_beg=k_beg,hy_dens_cell=hy_dens_cell,hy_dens_theta_cell=hy_dens_theta_cell](int idx){
