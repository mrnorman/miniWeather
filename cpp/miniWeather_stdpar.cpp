  ...
  std::for_each(std::execution::par,begin1,end1,[=](int idx)
      {
        int k  = idx % _nz;
        int ll = idx / _nz;
        state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + 0       ] = state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + _nx+hs-2];
        state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + 1       ] = state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + _nx+hs-1];
        state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + _nx+hs  ] = state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + hs     ];
        state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + _nx+hs+1] = state[ll*(_nz+2*hs)*(_nx+2*hs) + (k+hs)*(_nx+2*hs) + hs+1   ];
      });
  ////////////////////////////////////////////////////
