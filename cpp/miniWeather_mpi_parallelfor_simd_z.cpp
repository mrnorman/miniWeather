...
void compute_tendencies_x( realConst3d state , real3d const &tend , real dt , Fixed_data const &fixed_data ) {
  ...
  parallel_for( SimpleBounds<2>(zblocks,nx+1) , YAKL_LAMBDA (int kblk, int i) {
    ...
    Pack<real,simd_len> hy_dens_loc;
    Pack<real,simd_len> hy_dens_theta_loc;
    for (int ilane=0; ilane < simd_len; ilane++) {
      int k = min( zdim-1 , kblk*simd_len + ilane );
      hy_dens_loc      (ilane) = hy_dens_cell      (hs+k);
      hy_dens_theta_loc(ilane) = hy_dens_theta_cell(hs+k);
    }
    ...
    auto r = vals(ID_DENS) + hy_dens_loc;
    auto u = vals(ID_UMOM) / r;
    auto w = vals(ID_WMOM) / r;
    auto t = ( vals(ID_RHOT) + hy_dens_theta_loc ) / r;
    ...
    for (int ilane=0; ilane < simd_len; ilane++) {
      int k = std::min(zdim-1 , kblk*simd_len + ilane);
...
void compute_tendencies_z( realConst3d state , real3d const &tend , real dt , Fixed_data const &fixed_data ) {
  ...
  parallel_for( SimpleBounds<2>(zblocks,nx) , YAKL_LAMBDA (int kblk, int i) {
    ...
    Pack<real,simd_len> hy_dens_loc;
    Pack<real,simd_len> hy_dens_theta_loc;
    Pack<real,simd_len> hy_pressure_loc;
    for (int ilane=0; ilane < simd_len; ilane++) {
      int k = min( zdim-1 , kblk*simd_len + ilane );
      hy_dens_loc      (ilane) = hy_dens_int      (k);
      hy_dens_theta_loc(ilane) = hy_dens_theta_int(k);
      hy_pressure_loc  (ilane) = hy_pressure_int  (k);
    }
    //Compute the hyperviscosity coeficient
    ...
    auto w = vals(ID_WMOM) / r;
    auto t = ( vals(ID_RHOT) + hy_dens_theta_loc ) / r;
    auto p = C0*pow((r*t),gamm) - hy_pressure_loc;
    for (int ilane=0; ilane < simd_len; ilane++) {
      int k = min( zdim-1 , kblk*simd_len + ilane );
      if (k == 0 || k == nz) {
        w(ilane)                = 0;
        d3_vals(ID_DENS)(ilane) = 0;
      }
    }
